...
/* MEMP_NUM_UDP_PCB: the number of UDP protocol control blocks. One
   per active UDP "connection". Includes headroom for the mDNS
   responder's UDP PCB in addition to DHCP. */
 ...
 * field. Required so the mDNS responder and DHCP client can advertise a
 * name for the board (e.g. "mcxn947-board"). */
...
/* LWIP_IGMP==1: Enable IGMP, required so the interface can join the
 * multicast group mDNS depends on (224.0.0.251). */
#ifndef LWIP_IGMP
#define LWIP_IGMP 1
#endif
...
/* ---------- DNS options ---------- */
/* LWIP_DNS==1: Enable DNS client, needed only if the board itself
 * initiates outbound connections by hostname (e.g. NTP, MQTT broker,
 * cloud API). Not required for the board to act purely as an HTTP
 * server that others connect to, but harmless to leave on. */
#ifndef LWIP_DNS
#define LWIP_DNS 1
#endif
...
/* ---------- Socket options ---------- */
/* SO_REUSE==1: allow the HTTP server's listening socket to rebind
 * cleanly after a reset/reconnect instead of returning "address in
 * use". Recommended for any server socket. */
#ifndef SO_REUSE
#define SO_REUSE 1
#endif

/* ---------- mDNS options ---------- */
/* LWIP_MDNS_RESPONDER==1: advertise the board as <hostname>.local so
 * it can be reached in a browser without knowing its IP address.
 * Requires LWIP_IGMP and LWIP_NETIF_HOSTNAME above. */
